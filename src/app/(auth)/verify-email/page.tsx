...
const VerifyEmail = () => {
    ...
    const handleSubmit = async (event: React.FormEvent) => {
        ...
        try {
            setLoading(true);
            const response: ApiResponse = await axios.patch("/api/profile/verify", {
                email,
    ...
    // Runs once per cooldown cycle, not once per second.
    // Dependency is a boolean (timer === 0), so this effect only
    // re-fires when the countdown starts or finishes — not on every tick.
    // This avoids recreating setInterval 60 times per cycle, and avoids
    // auto-resetting the timer the instant it hits 0 (which made the
    // resend button effectively unclickable before).
    useEffect(() => {
        if (timer === 0) return;
